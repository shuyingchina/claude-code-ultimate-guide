...
- **Guide translation script: multi-language support** (`scripts/translate-guide.py`): added a `--lang` argument (`choices: fr, zh`, default `fr` to preserve prior behaviour). The system prompt is now built dynamically per target language, output goes to `guide/ultimate-guide.<lang>.md`, and the resume cache is namespaced per language (`.translation-cache-<lang>/`) so French and Chinese runs never collide. Simplified Chinese (`--lang zh`) includes extra guidance for idiomatic phrasing and CJK/inline-English spacing. Run with `python3 scripts/translate-guide.py --lang zh`. The cache `.gitignore` rule was widened to `.translation-cache-*/`.
- **Simplified Chinese guide** (`guide/ultimate-guide.zh.md`): full translation of the main guide (~26.4K lines). Structural parity with the English source verified — identical `##`/`###` heading counts (320/651) and table-row counts (2614); all code blocks, shell commands, file paths, and proper nouns preserved in English per the translation ruleset.
- **Simplified Chinese README** (`README.zh.md`): full translation of the root `README.md` (946 lines). Structural parity verified — identical `#`/`##`/`###` heading counts (5/22/33), table-row count (112), and code-fence count (26); all HTML/badges, shield.io URLs, code, paths, and proper nouns preserved in English per the translation ruleset.
- **Simplified Chinese translations for all other guide docs** (86 files, `guide/**/*.zh.md`): translated every remaining `.md` under `guide/` (core, security, ecosystem, roles, ops, workflows, diagrams, learning-path, cheatsheet, READMEs) following the `<name>.md` → `<name>.zh.md` convention. Each file split at heading boundaries, translated chunk-by-chunk, and reassembled with single-newline joins for exact line structure. Structural parity verified across all 86 files — `##`/`###` heading counts and table-row counts match the English sources, code/paths/proper nouns kept in English.
