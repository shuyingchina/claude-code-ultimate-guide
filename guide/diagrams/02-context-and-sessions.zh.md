...
description: "上下文区域、记忆层级、会话管理与全新上下文模式"
tags: [context, sessions, memory, optimization]
---

# 上下文与会话

Claude Code 如何在你的工作过程中管理上下文、记忆与会话。

---

### 上下文管理区域

你的上下文窗口分为 4 个不同的区域，每个区域需要采用不同的策略。了解自己处于哪个区域有助于防止上下文膨胀，并在长会话中保持响应质量。
...
Claude Code 拥有 6 种不同的记忆类型，各自具有不同的作用域与持久性。为每一类信息选择正确的记忆类型，是高效会话的关键。
...
> **来源**：[记忆系统](../ultimate-guide.md#memory-system) — 第 ~3160 行及 ~3986 行 | Auto-Memory：v2.1.59+（v3.30.0）

---

### 会话连续性 — 保存与恢复状态

会话不会在不同终端之间自动保留上下文。本图展示了如何保存状态并在新会话或新终端中恢复，从而支持异步工作流。
...
长会话会累积噪声，从而降低响应质量。本图展示了这种质量退化模式，以及推荐的、能保持性能的"专注会话"方法。
