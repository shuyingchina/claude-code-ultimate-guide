...
面向大规模部署 Claude Code 的团队的组织级模式——使用层级、MCP 审批工作流以及护栏配置。

> **目标读者**：技术负责人、工程经理、安全官。关于个人开发者安全，参见 [Security & Production](./08-security-and-production.md)。

---

### 治理风险层级——控制什么、何时控制

并非所有事项都需要重度治理。这棵决策树会根据实际风险，将你的场景路由到合适的控制级别——从个人开发工作流（最小化）到受监管环境（完整的合规体系）。
...
> **Source**：[Enterprise Governance](../security/enterprise-governance.md) — §1 治理边界划分，§4 护栏层级

---

### MCP 治理工作流

单个 MCP 的审查只需 5 分钟。组织级 MCP 治理则是一条五步流水线，确保已批准的服务器持续合规、版本被固定，且在部署前完成风险分级。
...
> **Source**：[MCP Governance Workflow](../security/enterprise-governance.md#3-mcp-governance-workflow) — §3.1 审批工作流

---

### 数据分级与 Claude Code 访问规则

数据分级决定了 Claude Code 被允许读取和处理的内容。在这里出错是影响最大的治理失误。四个级别、清晰的规则，RESTRICTED 没有任何例外。
...
> **Source**：[AI Usage Charter](../security/enterprise-governance.md#2-ai-usage-charter) — §2.1 数据分级
