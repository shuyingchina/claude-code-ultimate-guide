...
Model Context Protocol (MCP) 通过外部工具服务器扩展 Claude Code 的能力。

---

### MCP 服务器生态系统地图

MCP 生态系统有 4 类服务器——官方、社区开发工具、社区运维工具和本地。了解已有的服务器可以避免重复造轮子。
...
MCP 是一个运行在 stdio 或 SSE 之上的 JSON-RPC 协议。Claude Code 充当客户端，MCP 服务器作为工具提供方。下图展示了完整的请求-响应循环。
...
> **来源**: [架构: MCP](../core/architecture.md#mcp-architecture) — 约第 795 行

---

### MCP Rug Pull 攻击链

最危险的 MCP 攻击途径：工具描述中暗藏 prompt injection 的恶意工具。这就是为什么你只应安装经过审查的 MCP 服务器。
...
> **来源**: [安全: MCP 威胁](../security/security-hardening.md#mcp-threats) — 约第 33 行

---

### MCP 配置层级

MCP 服务器配置可以存在于 4 个优先级层级（实际为 3 个文件）。解析顺序决定了哪些服务器可用，以及谁可以覆盖谁。
...
> **来源**: [MCP 配置](../ultimate-guide.md#mcp-configuration) — 约第 6149 行
