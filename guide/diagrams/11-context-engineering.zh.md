...
如何在正确的时间用正确的信息填充 Claude 的上下文窗口——以及架构选择如何决定 Claude 能否始终如一地遵循你的规范。
...
上下文工程跨越三个不同的层级运作,它们具有不同的作用域和持久性。理解该使用哪一层可以避免最常见的错误:把所有内容塞进一个文件里。
...
> **来源**: [Context Engineering — Configuration Hierarchy](../core/context-engineering.md#3-configuration-hierarchy)

---

### 上下文预算与遵循度衰减

对 CLAUDE.md 规则的遵循度会随着文件体积增长而可预见地衰减。超过约 150 条规则后,模型开始选择性地忽略指令。路径限定(Path-scoping)是首要的解决方案——它在不损失覆盖范围的前提下将常驻上下文减少 40-50%。
...
> **来源**: [Context Budget](../core/context-engineering.md#2-the-context-budget) — 遵循度数据:HumanLayer 生产环境数据(采用结构化上下文后提升 15-25%)

---

### 单体架构 vs. 模块化架构

单体式 CLAUDE.md 是团队场景中最常见的失败模式。路径限定的模块通过只加载与当前任务相关的内容来解决这个问题。
...
> **来源**: [Modular Architecture](../core/context-engineering.md#4-modular-architecture) — 路径限定模式

---

### 规则归位决策树

每条新指令或规范都需要落在正确的层级。归位错误会浪费 token(过于全局化)或损失覆盖范围(限定过窄)。这棵决策树让选择变得明确。
...
> **来源**: [Rule Placement](../core/context-engineering.md#3-configuration-hierarchy) — 来自 §3 的决策树
