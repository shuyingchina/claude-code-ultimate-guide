...
在敏感环境和生产环境中安全运行 Claude Code 的模式。

---

### 安全三层防御模型

Claude Code 的纵深防御：预防层拦截大多数威胁，检测层捕获漏网之鱼，响应层限制爆炸半径。任何单一层都不足以独立支撑。
...
> **来源**: [Security Hardening](../security/security-hardening.md) — 完整指南

---

### 沙箱决策树

沙箱化会带来额外开销。使用这棵决策树来判断，对于你的具体情况，沙箱是强制必需、推荐使用，还是可选。
...
> **来源**: [Sandbox Native](../security/sandbox-native.md) — 约第 512 行

---

### 验证悖论

让 Claude 验证自己的工作是循环论证。产生这个 bug 的同一个模型，往往会在审查时同样漏掉它。这种反模式会引发生产事故。
...
> **来源**: [Production Safety](../security/production-safety.md) — 约第 639 行

---

### CI/CD 集成流水线

Claude Code 可以在 CI/CD 流水线中以非交互模式运行，对每一个 PR 自动执行代码审查、文档生成和质量检查。
...
> **来源**: [CI/CD Integration](../ultimate-guide.md#cicd-integration) — 约第 6835 行
